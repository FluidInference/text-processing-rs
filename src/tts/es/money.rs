//! Money TN tagger for Spanish.
//!
//! Converts written currency expressions to spoken Spanish:
...
struct Currency {
    ...
    /// Whether "uno" becomes "una" for this currency (feminine)
    feminine: bool,
}
...
const EURO: Currency = Currency {
    singular: "euro",
    plural: "euros",
    cent_singular: "centimo",
    cent_plural: "centimos",
    feminine: false,
};
...
const POUND: Currency = Currency {
    singular: "libra",
    plural: "libras",
    cent_singular: "penique",
    cent_plural: "peniques",
    feminine: true,
};
...
/// Parse a written money expression to spoken Spanish.
...
fn parse_prefix_currency(input: &str) -> Option<String> {
    ...
    // Check for scale suffix: "$2,5 millones"
    ...
    if let Some(scale_word) = scale {
        // With scale: "$2,5 millones" → "dos coma cinco millones de dolares"
        let decimal_sep = if amount_str.contains(',') {
            ','
        } else if amount_str.contains('.') {
            '.'
        } else {
            // No decimal: "$50 millones" → "cincuenta millones de dolares"
...
fn format_currency(dollars: i64, cents: i64, currency: &Currency) -> String {
    let dollar_words = if dollars == 1 && currency.feminine {
        "una".to_string()
    } else if currency.feminine {
        number_to_words(dollars)
    } else {
        // Masculine: "uno" → "un" before noun
        apocope_uno(&number_to_words(dollars))
    };

    if dollars == 0 && cents == 0 {
        return format!("cero {}", currency.plural);
    }

    if dollars == 0 {
        let cents_words = apocope_uno(&number_to_words(cents));
        let unit = if cents == 1 {
...
mod tests {
    ...
    #[test]
    fn test_euro_suffix() {
        assert_eq!(parse("5 €"), Some("cinco euros".to_string()));
        assert_eq!(parse("1 €"), Some("un euro".to_string()));
        assert_eq!(
            parse("5,50 €"),
            Some("cinco euros con cincuenta centimos".to_string())
        );
    }

    #[test]
    fn test_prefix_currencies() {
        assert_eq!(parse("$100"), Some("cien dolares".to_string()));
        assert_eq!(parse("£1"), Some("una libra".to_string()));
        assert_eq!(parse("€100"), Some("cien euros".to_string()));
    }

    #[test]
    fn test_dollar_with_cents() {
        assert_eq!(
            parse("$5.50"),
    ...
    fn test_large_amounts() {
        assert_eq!(
            parse("$2,5 millones"),
            Some("dos coma cinco millones de dolares".to_string())
        );
        assert_eq!(
            parse("$50 millones"),
            Some("cincuenta millones de dolares".to_string())
        );
    }

    #[test]
    fn test_trailing_dot() {
        assert_eq!(parse("$5."), Some("cinco dolares".to_string()));
        assert_eq!(parse("$1."), Some("un dolar".to_string()));
    }
